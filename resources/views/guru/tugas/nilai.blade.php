...
@push('script-bottom')
    <script>
        ...
        function hapusNilai(id) {
            const hapusForm = document.getElementById('hps-nilai-' + id);
            Swal.fire({
                title: 'Apakah anda yakin?',
                text: "nilai yang di hapus tidak dapat dikembalikan!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#3085d6',
                    ...
                    @forelse ($result as $item)
                        <tr>
                            ...
                            <td class="flex items-center gap-4 text-white">
                                ...
                                <form action="{{ route('nilai-guru.destroy', ['nilai_guru' => $item->id]) }}"
                                    id="hps-nilai-{{ $item->id }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button onclick="hapusNilai({{ $item->id }})" type="button"
                                        class="bg-sky-800 p-2 rounded-lg hover:bg-sky-600">
                                        Hapus Data
                                    </button>
                                </form>
                            </td>

    ...
    <div class="grid grid-cols-2 gap-4">
        <div class="px-14 w-full space-y-5">
            ...
            <div class="w-full max-h-72 overflow-scroll space-y-6">
                <div class="flex items-center">
                    <h1 class="flex-grow text-xl font-semibold">Daftar Materi</h1>
                    <h1 class="w-24 text-xl font-semibold">Aksi</h1>
                </div>

                @forelse ($materi_all as $item)
                    {{-- BUTTON --}}
                    <div class="flex items-center gap-3">
                        <a href="{{ route('materi-guru.show', ['materi_guru' => $item->id]) }}"
        ...
        <div class="px-4">
            <div class="w-full p-4 rounded-lg shadow-md">
                <h1 class="text-lg text-kuning text-center mb-6">Tambah Materi</h1>

                <form action="{{ route('materi-guru.update', ['materi_guru' => $materis->id]) }}" method="post"
                    ...
                    <div class="space-y-2">
                        <p>Judul Materi</p>
                        <input type="text" name="nama" class="w-full border rounded p-1" value="{{ $materis->nama }}">
                    </div>

...
@push('script-bottom')
    <script>
        ...
        const tambahSoalButton = document.querySelector('#tambah-soal');

        // Menangkap elemen form
        const form = document.querySelector('#form-soal');
        ...
        // Menambahkan event listener ke tombol "Tambah Soal"
        tambahSoalButton.addEventListener('click', function() {
            const labelSoal = document.createElement('p');
            labelSoal.textContent = `Soal ${counter}`;
            counter++;
            form.insertBefore(labelSoal, tambahSoalButton);

            // Buat elemen textarea baru untuk soal
            const textareaSoal = document.createElement('textarea');
            textareaSoal.name = 'soal[]';
            textareaSoal.className = 'border w-full rounded-lg px-4 py-2';
            textareaSoal.placeholder = 'Isi Soal';
            form.insertBefore(textareaSoal, tambahSoalButton);

            // Buat elemen input file baru untuk gambar soal
            const inputGambarSoal = document.createElement('input');
            inputGambarSoal.type = 'file';
            inputGambarSoal.name = 'gambar[]'; // gunakan [] jika Anda mengharapkan multiple files
            inputGambarSoal.className = 'w-full border rounded p-1 mt-2';
            form.insertBefore(inputGambarSoal, tambahSoalButton);
        });
    </script>
@endpush
    ...
    <div class="grid grid-cols-2 gap-4 mb-14">
        <div class="px-14 w-full space-y-5">
            ...
            <div class="w-full max-h-72 overflow-scroll space-y-6">
                <div class="flex items-center">
                    <h1 class="flex-grow text-xl font-semibold">Daftar kuis</h1>
                ...
                @forelse ($kategoris as $item)
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ route('opsi.show', ['opsi' => $item->id]) }}"
                        ...
                        <div class="w-24 flex items-center justify-between">
                            <a href="{{ route('opsi.edit', ['opsi' => $item->id]) }}">
                            ...
                            <div id="1-item" class="text-center space-y-1 text-merah cursor-pointer"
                                ...
                                <form action="{{ route('soal.destroy', ['soal' => $item->id]) }}" method="post"
                                    id="delete-{{ $item->id }}">
                                    @csrf
                                    @method('delete')
                                </form>
                ...
                @empty
                    ...
                    -center justify-center w-full h-36 bg-white rounded-lg shadow-md">
                        <p class="text-lg text-netral">Belum ada kuis</p>
                    </div>
                @endforelse
        ...
        <div class="px-4">
            <div class="p-4 rounded-lg shadow-md">
                <h1 class="text-xl text-kuning font-semibold text-center mb-6">Tambah Soal</h1>

                <form action="{{ route('opsi.store') }}" method="post" class="space-y-3" enctype="multipart/form-data"
                    id="form-soal">
                    @csrf
                    @method('POST')

                    <input type="hidden" name="kategori_kuis_id" value="{{ $id }}">
                    ...
                    @forelse ($soals as $item)
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="soal[]" class="border w-full rounded-lg px-4 py-2" placeholder="Isi Soal" readonly>{{ $item->soal }}</textarea>

                            {{-- OPSI  --}}
                        </div>

                        <div class="space-y-2">
                            <p class="font-bold">Gambar Soal {{ $loop->iteration }}</p>
                            <img src="{{ asset('storage/kuis/soal/gambar') . '/' . $item->gambar }}" class="w-20 h-20"
                                alt="">
                            {{-- <input type="file" name="gambar[]" class="w-full border rounded p-1" readonly> --}}
                        </div>

                        <div class="space-y-2">
                            @foreach ($item->opsi as $index => $opsi)
                                <div class="flex items-center gap-2">
                                    <input readonly type="text" class="border w-full rounded-lg px-4 py-2 grow" name="opsi[]"
                                        value="{{ $opsi->opsi }}">
                                    {{-- point --}}
                                    <input readonly type="text" class="border w-20 rounded-lg px-4 py-2 min-w-24" name="point[]"
                                        value="{{ $opsi->point }}">
                                    {{-- input hidden opsi id --}}
                                    <input readonly type="hidden" name="opsi_id[]" value="{{ $opsi->id }}">
                                </div>
                            @endforeach

                            @php
                                $jumlahInput = 5 - count($item->opsi); // Hitung berapa banyak input tambahan yang diperlukan
                            @endphp

                            @for ($i = 0; $i < $jumlahInput; $i++)
                                <div class="flex items-center gap-2">
                                    <input readonly type="text" class="border w-full rounded-lg px-4 py-2 grow" name="opsi[]"
                                        id="" placeholder="masukkan opsi">
                                    <input readonly type="text" class="border w-20 rounded-lg px-4 py-2 min-w-24" name="point[]"
                                        id="" placeholder="poin">
                                </div>
                            @endfor
                    ...
                    @empty
                        <div class="space-y-2">
                            <p>Belum ada soal</p>
                            <textarea name="soal[]" class="border w-full rounded-lg px-4 py-2" id="" placeholder="Isi Soal" readonly></textarea>
                        </div>
